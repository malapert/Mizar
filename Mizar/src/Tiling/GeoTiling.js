define(['../Utils/Utils', './Tile', '../Renderer/GeoBound', './HEALPixBase'], function (Utils, Tile, GeoBound, HEALPixBase) {
  ...
  GeoTile.prototype.generateVertices = function (elevations) {
    ...
    GeoTiling.prototype.generateLevelZeroTiles = function (config) {
        ...
        var latStep = (config.coordinateSystem.geoBound[3] - config.coordinateSystem.geoBound[1]) / this.level0NumTilesY;
        var lonStep = (config.coordinateSystem.geoBound[2] - config.coordinateSystem.geoBound[0]) / this.level0NumTilesX;

       for (var j = 0; j < this.level0NumTilesY; j++) {
            for (var i = 0; i < this.level0NumTilesX; i++) {
                var geoBound = new GeoBound(config.coordinateSystem.geoBound[0] + i * lonStep, config.coordinateSystem.geoBound[3] - (j + 1) * latStep, config.coordinateSystem.geoBound[0] + (i + 1) * lonStep, config.coordinateSystem.geoBound[3] - j * latStep);
