    ...
    var MouseNavigationHandler = function (options) {
        ...
        var _handleMouseDblClick = function (event) {
            if (event.button === 0) {
                ...
                var pos,geo;
                if (_navigation.planet) {
                  pos = _navigation.planet.renderContext.getXYRelativeToCanvas(event);
                  geo = _navigation.planet.getLonLatFromPixel(pos[0], pos[1]);
                } else if (_navigation.sky) {
                  pos = _navigation.sky.renderContext.getXYRelativeToCanvas(event);
                  geo = _navigation.sky.getLonLatFromPixel(pos[0], pos[1]);
                }
