define(function () {
    ...
    var GoogleMouseNavigationHandler = function (options) {
        ...
        var _handleMouseWheel = function (event) {
            _navigation.planet.publish("startNavigation");
            ...
            if (!_navigation.inertia) {
                ...
                pos = _navigation.planet.renderContext.getXYRelativeToCanvas(event);
                geo = _navigation.planet.getLonLatFromPixel(pos[0], pos[1]);
            }
            ...
            if (_navigation.inertia) {
                ...
                // Compute the new position of lon lat and pan the planet toward it
                if (geo) {
                    var pos2 = _navigation.planet.getPixelFromLonLat(geo[0], geo[1]);

                    var dx = pos[0] - pos2[0];
                    var widthHeightFactor = Math.round(_navigation.planet.renderContext.canvas.width / _navigation.planet.renderContext.canvas.height);
            ...
            _navigation.planet.publish("endNavigation");
            _navigation.planet.renderContext.requestFrame();
        ...
        var _handleMouseMove = function (event) {
            ...
            if (_pressedButton === 0) {
                ...
                if (_pressedGeo) {
                    _changeInertia = null;
                    inside = _navigation.planet.getLonLatFromPixel(pos[0], pos[1]);
                    if (inside) {
                        pos2 = _navigation.planet.getPixelFromLonLat(_pressedGeo[0], _pressedGeo[1]);
                        _dx = pos[0] - pos2[0];
                        _dy = pos[1] - pos2[1];
                        _navigation.pan(_dx, _dy);
                    }
                }
                // If the mouse not on the planet
                if (!_pressedGeo || !inside) {
                    if (Math.abs(_dx) > Math.abs(_dy)) {
                        _changeInertia = (pos[1] > (_navigation.planet.renderContext.canvas.height / 2)) ? -_dx : _dx;
                    }
                    else {
                        _changeInertia = (pos[0] > (_navigation.planet.renderContext.canvas.width / 2)) ? _dy : -_dy;
                    }
                    _navigation.rotate(_changeInertia, 0);
                    pos = _navigation.planet.renderContext.getXYRelativeToCanvas(event);
                    _pressedGeo = _navigation.planet.getLonLatFromPixel(pos[0], pos[1]);
                    if (_pressedGeo) {
                      _changeInertia = null;
                    }
                }
                ...
                if ((_slower % 3 === 0) && ( _slower > 1 )) {
                    ...
                    if (_dy > 0 && _dy > _dx) {
                        ...
                        if (_pressedGeo) {
                            ...
                            pos = [_navigation.planet.renderContext.canvas.clientLeft + (_navigation.planet.renderContext.canvas.clientWidth / 2), _navigation.planet.renderContext.canvas.clientTop + (_navigation.planet.renderContext.canvas.clientHeight / 2)];
                            pos2 = _navigation.planet.getPixelFromLonLat(_pressedGeo[0], _pressedGeo[1]);
        ...
        var _handleMouseDblClick = function (event) {
            if (event.button === 0) {
                var pos = _navigation.planet.renderContext.getXYRelativeToCanvas(event);
                var geo = _navigation.planet.getLonLatFromPixel(pos[0], pos[1]);
