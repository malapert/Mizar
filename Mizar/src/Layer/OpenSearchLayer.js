define(['../Renderer/FeatureStyle', '../Renderer/VectorRendererManager', '../Utils/Utils', './BaseLayer', '../Renderer/RendererTileData', '../Tiling/Tile'],
    function (FeatureStyle, VectorRendererManager, Utils, BaseLayer, RendererTileData, Tile) {
        ...
        OpenSearchLayer.prototype.launchRequest = function (tile, url) {
            ...
            if (this.maxRequests === this.freeRequests.length) {
                this.planet.publish("startLoad", this);
            }
            ...
            xhr.onreadystatechange = function (e) {
                ...
                if (xhr.readyState === 4) {
                    ...
                    if (self.maxRequests === self.freeRequests.length) {
                        self.planet.publish("endLoad", self);
                    }
                }
            };
            xhr.open("GET", url);
        ...
        OpenSearchLayer.prototype.setRequestProperties = function (properties) {
            ...
            for (var x in this.featuresSet) {
                ...
                for (var i = 0; i < featureData.tiles.length; i++) {
                    ...
                    this.planet.vectorRendererManager.removeGeometryFromTile(this, feature.geometry, tile);
                }
            }

            // Clean old results
            var self = this;
            this.planet.tileManager.visitTiles(function (tile) {
        ...
        OpenSearchLayer.prototype.addFeature = function (feature, tile) {
            ...
            this.planet.vectorRendererManager.addGeometryToTile(this, feature.geometry, style, tile);
        };
        ...
        OpenSearchLayer.prototype.modifyFeatureStyle = function (feature, style) {
            ...
            if (featureData) {
                for (var i = 0; i < featureData.tiles.length; i++) {
                    var tile = featureData.tiles[i];
                    this.planet.vectorRendererManager.removeGeometryFromTile(feature.geometry, tile);
                    this.planet.vectorRendererManager.addGeometryToTile(this, feature.geometry, style, tile);
                }
            }
        };
            ...
            for (var i = 0; i < features.length; i++) {
                ...
                switch (currentFeature.geometry.type) {
                    case "Point":
                        ...
                        /*if ( "EQ" != this.planet.tileManager.imageryProvider.tiling.coordSystem )
                         {
                         currentFeature.geometry.coordinates = CoordinateSystem.convert(currentFeature.geometry.coordinates, this.planet.tileManager.imageryProvider.tiling.coordSystem, "EQ");
                         }*/
                    ...
                    case "Polygon":
                        ...
                        for (var j = 0; j < ring.length; j++) {
                            ...
                            /*if ( "EQ" != this.planet.tileManager.imageryProvider.tiling.coordSystem )
                             {
                             ring[j] = CoordinateSystem.convert(ring[j], this.planet.tileManager.imageryProvider.tiling.coordSystem, "EQ");
                             }*/
