 ...
 */
define(["jquery", "gw/Utils/UtilsCore", "service/NameResolver", "underscore-min", "text!templates/nameResolverResult.html", "gw/Layer/LayerManager", "jquery.ui", "jquery.once"],
    function ($, UtilsCore, NameResolver, _, nameResolverResultHTMLTemplate, LayerManager) {
                ...
                for (var i = 0; i < response.features.length; i++) {
                    if(response.features[i].properties.type == "layer") {
                        ...
                        output += nameResolverResultTemplate({
                            ...
                            lon: 0,
                            lat: 0,
                            type: mizar.activatedContext.planet.coordinateSystem.type,
                            index : i
                        });
                        firstLayer = false;
                    } else {
                        var astro = UtilsCore.formatCoordinates([response.features[i].geometry.coordinates[0], response.features[i].geometry.coordinates[1]]);
                        output += nameResolverResultTemplate({
                            first: firstObject,
                            properties: response.features[i].properties,
                            lon: astro[0],
