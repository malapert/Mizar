 ...
 */
define(["jquery", "service/ReverseNameResolver", "./IFrame", "gw/Utils/UtilsCore", "./dialog/ErrorDialog", "underscore-min", "text!templates/featureDescription.html", "text!templates/descriptionTable.html", "jquery.ui"],
    function ($, ReverseNameResolver, IFrame, UtilsCore, ErrorDialog, _, featureDescriptionHTMLTemplate, descriptionTableHTMLTemplate) {
...
// Template generating the table of properties of choosen feature
        ...
        function _handleMouseUp(event) {
            ...
            if (diff > 500 && Math.abs(mouseXStart - event.clientX) < epsilon && Math.abs(mouseYStart - event.clientY) < epsilon) {
                var equatorial = [];
                geoPick = context.planet.getLonLatFromPixel(event.clientX, event.clientY);
        ...
        function _onCoordinatePicked(event) {
            ...
            var astro = UtilsCore.formatCoordinates([geoPick[0], geoPick[1]]);

            if (context.planet.coordinateSystem.type == "EQ") {
                $("#coordinatesInfo").html("<em>Right ascension:</em><br/>&nbsp;&nbsp;&nbsp;&nbsp;" + astro[0] +
                    "<br/><em>Declination :</em><br/>&nbsp;&nbsp;&nbsp;&nbsp;" + astro[1]);
            } else if (context.planet.coordinateSystem.type == "GAL") {
            ...
            if (context.configuration.debug && context.planet.tileManager.tiling.findInsideTile) {
                var selectedTile = context.planet.tileManager.getVisibleTile(geoPick[0], geoPick[1]);
