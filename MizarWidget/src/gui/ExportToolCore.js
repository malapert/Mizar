...
define(["jquery", "underscore-min", "jszip", "saveAs"],
    function ($, _, JSZip, saveAs) {

        var self, planet, layers, availableLayers;
        ...
        function filterServicesAvailableOnLayers() {
            ...
            _.each(layers, function (layer) {
                if (layer.getVisible() && layer.category !== "Other" && layer.category !== "Solar System"
        ...
        function exportSelection(event) {
            ...
            _.each(dataLayers, function (dataLayer, index) {
                for (var i = 0; i < self.coordinates.length; i++) {
                    var tile = planet.tileManager.getVisibleTile(self.coordinates[i][0], self.coordinates[i][1]);
