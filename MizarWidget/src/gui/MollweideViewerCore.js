 ...
 */
define(["jquery", "gw/Utils/UtilsCore", "gw/Renderer/Ray", "gw/Renderer/glMatrix"], function ($, UtilsCore, Ray) {
    ...
    function computeMollweidePosition(pos) {
        var coordinateSystem = planet.coordinateSystem;
    ...
    function updateNavigation(moll) {
        ...
        // Update navigation
        planet.coordinateSystem.fromGeoTo3D(geo, navigation.center3d);
    ...
    function updateMollweideFov() {
        ...
        var stepX = planet.renderContext.canvas.clientWidth / (tesselation - 1);
        var stepY = planet.renderContext.canvas.clientHeight / (tesselation - 1);
        ...
        for (var i = 0; i < tesselation; i++) {
            ...
            for (var j = 0; j < tesselation; j++) {
                // Height
                ray = Ray.createFromPixel(planet.renderContext, i * stepX, j * stepY);
                pos3d = ray.computePoint(ray.sphereIntersect([0, 0, 0], planet.coordinateSystem.geoide.radius));
        ...
        var fovx = UtilsCore.roundNumber(fov[0], 2);
        fovx = planet.coordinateSystem.fromDegreesToDMS(fovx);
        var fovy = UtilsCore.roundNumber(fov[1], 2);
        fovy = planet.coordinateSystem.fromDegreesToDMS(fovy);
