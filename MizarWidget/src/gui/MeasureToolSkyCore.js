...
define(["jquery", "underscore-min", "gw/Utils/Utils", "gw/Layer/VectorLayer", "gw/Renderer/Ray", "gw/Utils/Numeric", "gw/Renderer/FeatureStyle", "gw/Renderer/glMatrix"],
    function ($, _, Utils, VectorLayer, Ray, Numeric, FeatureStyle) {

        var planet, navigation, onselect, scale, measureLayer, self, dragging;
        ...
        function _handleMouseDown(event) {
            ...
            self.geoPickPoint = planet.getLonLatFromPixel(self.pickPoint[0], self.pickPoint[1]);
        ...
        function _handleMouseUp(event) {
            ...
            var stopPickPoint;
            if (event.type.search("touch") >= 0) {
                stopPickPoint = planet.getLonLatFromPixel(event.changedTouches[0].clientX, event.changedTouches[0].clientY);
            }
            else {
                stopPickPoint = planet.getLonLatFromPixel(event.layerX, event.layerY);
            }

            // Find angle between start and stop vectors which is in fact the radius
            var dotProduct = vec3.dot(vec3.normalize(planet.coordinateSystem.fromGeoTo3D(stopPickPoint)), vec3.normalize(planet.coordinateSystem.fromGeoTo3D(self.geoPickPoint)));
        ...
        function _handleMouseMove(event) {
            ...
            self.secondGeoPickPoint = planet.getLonLatFromPixel(self.secondPickPoint[0], self.secondPickPoint[1]);
            ...
            var dotProduct = vec3.dot(vec3.normalize(planet.coordinateSystem.fromGeoTo3D(self.secondGeoPickPoint)), vec3.normalize(planet.coordinateSystem.fromGeoTo3D(self.geoPickPoint)));
        ...
        function computeIntersection(points) {
            ...
            for (var i = 0; i < points.length; i++) {
                ...
                var pos3d = ray.computePoint(ray.sphereIntersect(worldCenter, planet.coordinateSystem.geoide.radius));
                points[i] = planet.coordinateSystem.from3DToGeo(pos3d);
            }
        ...
        function updateMeasure() {
            ...
            var center3d = ray.computePoint(ray.sphereIntersect([0, 0, 0], planet.coordinateSystem.geoide.radius));

            var geoCenter = planet.coordinateSystem.from3DToGeo(center3d);
            this.measureLabel = {
                geometry: {
                    type: "Point",
                    gid: "measureShape",
                ...
                properties: {
                    style: new FeatureStyle({
                        label: planet.coordinateSystem.fromDegreesToDMS(this.geoDistance),
                        //label: planet.coordinateSystem.from3DToGeo(geoCenter),
                        fillColor: [1, 1, 1, 1],
                        zIndex: 2
                    })
                }
        ...
        return {
            init: function (options) {
                ...
                planet.addLayer(measureLayer, options.planetLayer);
